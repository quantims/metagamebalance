...
#Path to file of game matrix; change this for your own use
...
gamemat = np.loadtxt(r'C:\Users\timrs\OneDrive\Miscellaneous Projects\Scissors Beats Rock\ssbmgamemat.csv', delimiter=',')

#Adjust these two values for your own needs
stdev = .17#set standard deviation. This is how much the matchup matrix varies between players; bigger means the game differs more from player to player
nsims = 10000 #the number of simulations we'll make
...
for n in sims:
    ...
    for i in dimension:
        for j in dimension:
            gamematn[i,j] = np.random.normal(gamematrix[i,j],stdev)
    msne(gamematn)
    #print(gamematn)
#print(usageresults)
